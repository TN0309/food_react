...
import HomTom from '../../components/HomeTom/HomTom'
import './Home.scss'
...
function Home() {
  ...
  return (
     ...
     <HomTom setVal={setVal}></HomTom>
          <ul className='home_nav'>
        {
          a.map((item, index)=>(
            <li onClick={()=> setActive(item)} key={index} className={active === item ?'home_item active' : 'home_item' }>
            {item}
          </li>
          ))
        }     
      ...
      <ul className="home__list">
        ...
        <Order render={render} setRender={setRender}></Order>
    </div>
